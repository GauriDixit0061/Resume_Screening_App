...
def cleanResume(txt):
    cleanText = re.sub(r'http\S+\s', ' ', txt)  # Raw string
    ...
    cleanText = re.sub(r'[' + special_chars + ']', ' ', cleanText)  # No `%` format needed

    cleanText = re.sub(r'\s+', ' ', cleanText)  # Remove extra spaces
